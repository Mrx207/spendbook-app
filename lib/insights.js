...
export function reconcile(txns, tolerance = 1) {
  ...
  for (const [account, list] of Object.entries(byAccount)) {
    // Oldest first. Same-day order used to be decided by sorting the balance
    // itself ascending, which is backwards on any day of debits - balance
    // falls as the day goes on, so the last transaction landed first and the
    // first landed last, and every adjacent pair on that day then compared a
    // transaction against a balance that came after it rather than before.
    // That produced a gap-shaped result for every single pair on the day,
    // none of them real. Time, when the bank gave one, is the only signal
    // that is not circular; falling back to when the row was stored keeps
    // a statement's original row order, which is chronological as exported.
