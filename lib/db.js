...
export async function ensureSchema() {
  ...
  // Marks categories the user created, so re-seeding the shipped ones never
  // touches them and only they can be deleted.
  await sql`ALTER TABLE categories ADD COLUMN IF NOT EXISTS custom BOOLEAN NOT NULL DEFAULT FALSE`;
  ...
  // The balance the bank reported after this transaction, which is what lets
  // the ledger be checked for anything that never arrived.
  await sql`ALTER TABLE transactions ADD COLUMN IF NOT EXISTS balance NUMERIC`;

  // One bulk categorise can rewrite hundreds of rows, so the previous values
  // are kept long enough to put them back.
  await sql`CREATE TABLE IF NOT EXISTS undo_log (
    id TEXT PRIMARY KEY, label TEXT NOT NULL, payload JSONB NOT NULL,
    created_at TIMESTAMPTZ DEFAULT now()
  )`;
  ...
  for (const c of DEFAULT_CATEGORIES) {
    // Only kind and excluded are reconciled - a renamed or recoloured category
    // keeps the user's version.
    await sql`INSERT INTO categories (id,name,icon,color,budget,excluded,kind,custom)
      VALUES (${c.id},${c.name},${c.icon},${c.color},${c.budget},${!!c.excluded},${c.kind},FALSE)
